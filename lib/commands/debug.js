...
function debug(options) {
    return new Promise((resolve, reject) => {
        ...
        const exeCmds = mountBuildCmds(dirPath, mainPath, mainDirPath, debugPath, generatedMainJsFilename, generatedStyleCssFilename, { minify: false });
        const runServer = (callback) => {
            ...
            var rewriter = function (req, res, next) {
              if (existsSync(path.resolve(debugPath, (url.parse(req.url).pathname).replace(/^\/+/, '')))) {
                  next();
                  return;
              }
              req.url = "/index.html";
