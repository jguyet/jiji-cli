...
function _new(options) {
    return new Promise((resolve, reject) => {
        const projectName = readlineSync.question("Which project name? $> ");
        const devices = ['Browser', 'Mobile'];
        const devicesGithubRepositories = ["https://github.com/jguyet/jiji-start.git", ""];
        const deviceIndex = readlineSync.keyInSelect(devices, 'Which platform?');
        ...
        const exeCmds = [
            ...
            },
            {
                ...
                cmd: `rm -rf "${path.resolve(projectPath, '.git')}"`
            }
        ];
